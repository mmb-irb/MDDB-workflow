...
class Dataset:
    ...
    def get_project_directories(self) -> list[str]:
        """Retrieve the list of project directories from the dataset configuration."""
        dirs = self._resolve_directory_patterns(self.config.get('projects', []))
        ignore_dirs = self._resolve_directory_patterns(self.config.get('ignore', []))
        dirs = [d for d in dirs if d not in ignore_dirs]
        if not dirs:
            raise ValueError("No project directories found in the dataset configuration.")
        return dirs

    def get_groups(self) -> dict[str, list[str]]:
        """Retrieve the groups of project directories from the dataset configuration."""
        groups = {}
    ...
    def status(self) -> pd.DataFrame:
        """Retrieve last line from logs from all project directories as a pandas DataFrame.
    ...
    def launch_workflow(self,
        ...
        for project_dir in self.project_directories:
            ...
            if slurm:
                ...
                subprocess.run(
                    f"{job_script_path} 2>&1 | tee {log_file}",
                    cwd=project_dir,
                    shell=True
                )
