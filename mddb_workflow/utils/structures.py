...
class Atom:
    ...
    def get_residue_index(self) -> int:
        """Get the atom residue index according to parent structure residues.
...
class Residue:
    ...
    def get_chain_index(self) -> int:
        """Get the chain index of the residue according to parent structure."""
...
class Chain:
    ...
    def set_index(self, index: int):
        """Set the residue index according to parent structure residues."""
    ...
    def get_residues(self) -> list['Residue']:
        """Get the residues in this chain.
...
class Structure:
    ...
    def generate_pdb(self, show_warnings: bool = True):
        ...
        if len(self.residues) >= 65536:
            if show_warnings: warn('More than 65536 residues found. Cycling residue numeration to fit to PDB 4-character limit.')
...
def filter_pdb_model(pdb_content: str, model: int) -> str:
    ...
    model_header_regex = r'^MODEL\s*([0-9]*)\s*$'
    ...
    for line in pdb_lines:
        # Skip any line which is not a model line
        model_line = re.match(model_header_regex, line)
        if not model_line: continue
        # If the model is not the model we aim for then skip, but record the number for the logs
        model_number = int(model_line[1])
        found_models.add(model_number)
        if model_number != model and model != -1:
            continue
        # If we just found the header
        filtered_pdb_content = line
        break
    # If there are no models then return the whole PDB content
    if len(found_models) == 0:
        return pdb_content
    # If we did not find the header then stop here
    if not filtered_pdb_content:
        raise RuntimeError(f'Could not find model "{model}" header. '
