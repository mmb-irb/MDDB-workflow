...
def terminal_execution(trajectory_input,topology_input,strand_indexes,sequence):
    ...
    # Enter inside helicalparameters folder to execute Canals software as all output generated by Curves+ is insidem and Canals needs the Curves+ output as input
    ...
    level1 = 0
    level2 = 0
    # We just want to compute series files, but if you want more output just use the link written before and include them in the instructions similarly to series input
...
def send_files(sequence,frames_limit):
    ...
    for fi in os.listdir():
        if fi.endswith(".ser"): # Check that we are going to analyze the correct files, that have .ser extension
            ...
            if helpar in ["shear","stagger","stretch","buckle","opening","propel","inclin","tip","xdisp","ydisp", "majw","majd","minw","mind","rise","shift","slide","roll","tilt","twist"]:
            ...
            if helpar in ['alphac', 'alphaw', 'betac', 'betaw', 'gammac', 'gammaw', 'deltac', 'deltaw', 'epsilc', 'epsilw', 'zetac', 'zetaw', 'chic', 'chiw', 'phasec', 'phasew']:
                files_allbackbones.append(fi)
    information_dictionary = {'avg_res':{'backbone':{},'grooves':{},'axisbp':{},'intrabp':{},'interbp':{}},'ts':{'backbone':{},'grooves':{},'axisbp':{},'intrabp':{},'interbp':{}}}
    ...
    information_dictionary1 = flow_files_average(files_averages,information_dictionary,sequence) # Call function to distribute the different files to compute their averages
    information_dictionary2 = flow_files_backbone(files_backbones,information_dictionary1) # Call function to distribute the files related to backbone torsions and perform different calculcations to each
    info_dictionary = flow_files_timeseries(files_averages,files_allbackbones,information_dictionary2,frames_limit) # Call function to distribut all files and compute Time Series in all of them
...
def flow_files_average(files,info_dict,sequence):
    for file in files: # Iterate over all files that must be analyzed
        ...
        baselen = checking(helpword) # Obtain baselen value depending on the file
        dataframe = read_series(file) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
        df1,df2 = average_std(dataframe,baselen,sequence)
        if helpword in ["roll","tilt","twist","rise","shift","slide"]: # INTER BASEPAIR BLOCK
            info_dict['avg_res']['interbp'][helpword] = {'avg':{},'std':{}}
            info_dict['avg_res']['interbp'][helpword]['avg'] = df1.T.values.tolist() #  we want to store all the information regarding the averages 
            info_dict['avg_res']['interbp'][helpword]['std'] = df2.T.values.tolist() #  we want to store all the information regarding the standard deviations

        elif helpword in ["shear","stagger","stretch","buckle","opening","propel"]: # INTRA BASEPAIR BLOCK
            info_dict['avg_res']['intrabp'][helpword] = {'avg':{},'std':{}}
            info_dict['avg_res']['intrabp'][helpword]['avg'] = df1.T.values.tolist() #  we want to store all the information regarding the averages 
            info_dict['avg_res']['intrabp'][helpword]['std'] = df2.T.values.tolist() #  we want to store all the information regarding the standard deviations

        elif helpword in ["xdisp","ydisp","inclin","tip"]: # AXIS BASEPAIR BLOCK
            info_dict['avg_res']['axisbp'][helpword] = {'avg':{},'std':{}} 
            info_dict['avg_res']['axisbp'][helpword]['avg'] = df1.T.values.tolist() #  we want to store all the information regarding the averages 
            info_dict['avg_res']['axisbp'][helpword]['std'] = df2.T.values.tolist() #  we want to store all the information regarding the standard deviations

        else:
            info_dict['avg_res']['grooves'][helpword] = {'avg':{},'std':{}} # GROOVES BLOCK
            info_dict['avg_res']['grooves'][helpword]['avg'] = df1.T.values.tolist() #  we want to store all the information regarding the averages 
            info_dict['avg_res']['grooves'][helpword]['std'] = df2.T.values.tolist() #  we want to store all the information regarding the standard deviations
...
######## THERE ARE 3 DIFFERENT FUNCTIONS EACH ONE COMPUTING DIFFERENT ASPECTS OF THE FLEXIBILITY IN THE BACKBONE (SUGAR PUCKERING, CANONICAL ALPHA/GAMMA AND BI/BII POPULATIONS)

# FUNCTION TO CONTROL 
def flow_files_backbone(files,info_dict):
    puckering_files = [] # Store files related to Puckering computations
    canonicalg_files = [] # Store files related to Canonical Alpha Gamma computations
    bipopulations_files = [] # Store files related to BI and BII populations
    for file in files: # Iterate over all files that must be analyzed
        word = file.split('.')
        helpword = word[0].split('_')[-1].lower()
        if helpword in ['phasec','phasew']: # Sugar Puckering
            puckering_files.append(file)
        elif helpword in ['alphac','alphaw','gammac','gammaw']: # Canonical Alpha/Gamma
            canonicalg_files.append(file)
        elif helpword in ['epsilc','epsilw','zetac','zetaw']: # BI/BII Population
            bipopulations_files.append(file)
    info_dict['avg_res']['backbone'] = {'puckering':{'north':{},'east':{},'west':{},'south':{}},'bi':{},'bii':{},'canonical_alphagamma':{}}
    npop, epop, wpop, spop = sugar_puckering(puckering_files) # Call function to perform make computations regarding Sugar Puckering
    ...
    canon = canonical_alphagamma(canonicalg_files) # Call function to perform make computations regarding Canonical Alpha Gamma
    info_dict['avg_res']['backbone']['canonical_alphagamma'] = canon.T.values.tolist()
    b1,b2 = bi_populations(bipopulations_files) # Call function to perform make computations regarding BI and BII populations
...
def sugar_puckering(pcukfiles):
    ...
    phasec = read_series(phasec) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    phasew = read_series(phasew) # Convert the files into Pandas dataframe to make the computations and data manipulation easily

    phasec = fix_angles(phasec) # Fix angles that are negative 
    phasew = fix_angles(phasew) # Fix angles that are negative
...
def canonical_alphagamma(canonfiles):
    ...
    alphac = read_series(alphac) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    alphaw = read_series(alphaw) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    gammac = read_series(gammac) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    gammaw = read_series(gammaw) # Convert the files into Pandas dataframe to make the computations and data manipulation easily

    alphac = fix_angles2(alphac) # Fix angles that are negative and over 360 degrees
    alphaw = fix_angles2(alphaw) # Fix angles that are negative and over 360 degrees
    gammac = fix_angles2(gammac) # Fix angles that are negative and over 360 degrees
    gammaw = fix_angles2(gammaw) # Fix angles that are negative and over 360 degrees
...
def bi_populations(bfiles):
    ...
    epsilc = read_series(epsilc) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    epsilw = read_series(epsilw) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    zetac = read_series(zetac) # Convert the files into Pandas dataframe to make the computations and data manipulation easily
    zetaw = read_series(zetaw) # Convert the files into Pandas dataframe to make the computations and data manipulation easily

    diff_epsil_zeta = angles_diff_ze(epsilc,zetac,epsilw,zetaw) # Compute differences between Epsilon and Zeta values
...
def flow_files_timeseries(files_average,files_backbone,info_dict,frames_limit):
    for file in files_average: # Iterate over all files that must be analyzed
        ...
        if helpword in ["roll","tilt","twist","rise","shift","slide"]: # Store all computations from files that are related to the block Inter Basepair 
            info_dict['ts']['interbp'][helpword] = {}
            info_dict['ts']['interbp'][helpword] = df.T.values.tolist()  # Store all information 


        elif helpword in ["shear","stagger","stretch","buckle","opening","propel"]: # Store all computations from files that are related to the block Intra Basepair
            info_dict['ts']['intrabp'][helpword] = {}
            info_dict['ts']['intrabp'][helpword] = df.T.values.tolist() # Store all information 



        elif helpword in ["xdisp","ydisp","inclin","tip"]: # Store all computations from files that are related to the block Axis Basepair
            info_dict['ts']['axisbp'][helpword] = {}
            info_dict['ts']['axisbp'][helpword] = df.T.values.tolist() # Store all information 


        else:
            info_dict['ts']['grooves'][helpword] = {} # Store all computations from files that are related to the block Grooves
            info_dict['ts']['grooves'][helpword] = df.T.values.tolist() # Store all information 
    ...
    for file2 in files_backbone:
        ...
        info_dict['ts']['backbone'][helpword] = df1.T.values.tolist() # Store all information
